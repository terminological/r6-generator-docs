...
#' Version: 1.1.0
#' 
#' Generated: 2024-05-16T16:22:15.466275202
#'
#' @details
...
FeatureTest = R6::R6Class("FeatureTest", public=list(
	...
	#' Add some numbers (1).
	#'   
	#'   The doSum function description = it adds two
	#'   numerics
	#' @param a the A parameter, can be NA - (java expects a RNumeric)
	#' @param b the B parameter - (java expects a RNumeric)
	#' @return RNumeric
	#' A+B of course, NAs in inputs are converted to null in Java. This
	#'   catches the resulting NPE in java idiom and returns an explicit NA.
	#'   
	#'   This only matters if you care about the difference between NA_real_
	#'   and NaN in R.
	#' @examples
	#' library(roxygen2)
	#' library(devtools)
	...
	doSum = function(a, b) {
		...
		tmp_out = .jcall(
			self$.jobj, 
			returnSig = "Luk/co/terminological/rjava/types/RNumeric;", 
			method = "doSum",
			tmp_a,
			tmp_b, 
			check=FALSE
	...
	#'   It
	#'   should throw an error if given something that cannot be coerced to an
	#'   integer. 
	...
	#'   
	#'   A fluent method which updates the
	#'   message in this object, returning the
	#'   same object. This is
	#'   differentiated from factory methods which produce a new
	#'   instance of
	#'   the same class by checking to see if the returned Java object is
	#'   equal
	#'   to the calling Java object.
	#' @param message the message is a string - (defaulting to
...
world") {
		...
		tmp_out = .jcall(
			self$.jobj, 
			returnSig = "Luk/co/terminological/rjava/test/FeatureTest;", 
		...
		# convert java object back to R. Wrapping to R6 as needed. 
		out = self$.api$.fromJava$FeatureTest(tmp_out);
		if(is.null(out)) return(invisible(out));
		return(out);
	},
	...
	#' @description 
	#' factoryMethod: 
	#' A factory or builder method which constructs an object of another
	#'   class from some parameters
	...
	factoryMethod = function(a, b=as.character(Sys.Date())) {
		...
		# convert java object back to R. Wrapping to R6 as needed. 
		out = self$.api$.fromJava$MoreFeatureTest(tmp_out);
		if(is.null(out)) return(invisible(out));
		return(out);
	},
	...
	#' The result of a `get()` call will be a RCharacter
	#' 
	asyncCountdown = function() {
		# copy parameters into Java
		# construct a JFuture R6 object as return value.
		out = RFuture$new(
			r6obj = self,
			converter = self$.api$.fromJava$RCharacter,
			returnSig = "Luk/co/terminological/rjava/types/RCharacter;", 
